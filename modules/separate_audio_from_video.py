...
class SeparateAudioFromVideo:
    ...
    def process_video_to_audio(self) -> str:
        ...
        utils = self.utils
        audio_format_is_valid = CompatibleAudioFormats.has_value(
            media_data.extracted_audio_format
        )
        video_format_is_valid = CompatibleVideoFormats.has_value(
            media_data.source_video_format
        )
        ...
        if not both_formats_valid:
            raise ValueError(
                "Invalid video and audio formats, please check compatibility"
            )
        ...
        if not audio_format_is_valid or not video_format_is_valid:
            ...
            raise ValueError(
                f"Invalid {video, audio} format, please check compatibility"
            )
